import type { JsonValue } from "../../tests/ct/servers/types";

import { EVM_WALLET_MOCK, SOLANA_RPC_URL, SOLANA_WALLET_MOCK, TRON_WALLET_MOCK } from "./constants";
...
export const installWalletMocks = (win: Window): void => {
    const evmState = {
        balances: {
            [EVM_WALLET_MOCK.sender.toLowerCase()]: BigInt(EVM_WALLET_MOCK.initialSenderBalanceWei),
            [EVM_WALLET_MOCK.recipient.toLowerCase()]: BigInt(EVM_WALLET_MOCK.initialRecipientBalanceWei),
        },
        counter: 0,
    };
    ...
    const ethereumProvider: EvmProvider = {
        isMetaMask: true,
        selectedAddress: EVM_WALLET_MOCK.sender,
        request: async ({ method, params }: EvmRequest) => {
            if (method === "eth_requestAccounts" || method === "eth_accounts") {
                return [EVM_WALLET_MOCK.sender];
            }

            if (method === "eth_chainId") {
                return EVM_WALLET_MOCK.chainId;
            }
            ...
            if (method === "eth_sendTransaction") {
                ...
                const from = String(transaction?.from ?? EVM_WALLET_MOCK.sender).toLowerCase();
                const to = String(transaction?.to ?? EVM_WALLET_MOCK.recipient).toLowerCase();
    ...
    const solanaProvider = {
        isSolflare: true,
        isPhantom: false,
        publicKey: createPublicKey(SOLANA_WALLET_MOCK.sender),
        connect: async () => ({
            publicKey: createPublicKey(SOLANA_WALLET_MOCK.sender),
        }),
        disconnect: async () => {},
    ...
    const signTransaction = async (transaction: TronSignableTransaction) => {
        return {
            ...transaction,
            signature: [`cypress-tron-signature-${Date.now()}`],
        };
    };
    ...
    const tronLinkProvider: TronLinkProvider = {
        ...
        request: async ({ method }) => {
            if (method === "tron_requestAccounts") {
                return {
                    code: 200,
                    message: "ok",
                    address: TRON_WALLET_MOCK.sender,
                };
            }
        ...
        tronWeb: {
            defaultAddress: {
                base58: TRON_WALLET_MOCK.sender,
                hex: TRON_WALLET_MOCK.senderHex,
            },
            trx: {
    ...
    Object.defineProperty(win, "solana", {
        configurable: true,
        value: solanaProvider,
        writable: true,
    });
    Object.defineProperty(win, "solflare", {
        configurable: true,
        value: solanaProvider,
        writable: true,
    });
    Object.defineProperty(win, "phantom", {
        configurable: true,
        value: phantomProvider,
        writable: true,
    });
    Object.defineProperty(win, "tronLink", {
        configurable: true,
        value: tronLinkProvider,
        writable: true,
    });
    Object.defineProperty(win, "tronWeb", {
        configurable: true,
