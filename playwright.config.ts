...
export default defineConfig({
    ...
    webServer: [
        ...
        ...SYNDICATION_SERVERS.map((server) => ({
            command: `node --import tsx tests/e2e/servers/mockGraphqlServer.ts --port ${server.port} --fixture ${path.resolve(rootDir, `tests/e2e/fixtures/graphql/${server.name}.scenarios.json`)}`,
        ...
        {
            command: `node --import tsx tests/e2e/servers/mockSolanaRpcServer.ts --port ${SOLANA_RPC_PORT}`,
        ...
        {
            command: `node --import tsx tests/e2e/servers/mockTronRpcServer.ts --port ${TRON_RPC_PORT}`,
